...
from criteria_comparing_sets_pcs.general_utils import iterate_in_chunks, unit_cube_grid_point_cloud
...
sys.path.append(osp.join(ROOT_DIR, 'external/structural_losses'))
from tf_nndistance import nn_distance
from tf_approxmatch import approx_match, match_cost    

def minimum_mathing_distance_tf_graph(n_pc_points, batch_size=None, normalize=True, sess=None, verbose=False, use_sqrt=False, use_EMD=True):
...
def minimum_mathing_distance(sample_pcs, ref_pcs, batch_size, normalize=True, sess=None, verbose=False, use_sqrt=False, use_EMD=True):
    ...
    return {"mmd": mmd, "matched_dists": matched_dists}


def coverage(sample_pcs, ref_pcs, batch_size, normalize=True, sess=None, verbose=False, use_sqrt=False, use_EMD=True, ret_dist=False):
    ...
    if ret_dist:
        return {"cov": cov, "matched_gt": matched_gt, "matched_dist": matched_dist}
    else:
        return {"cov": cov, "matched_gt": matched_gt}
